...
{
public:
	NumericProperty(const std::string& name, T min, T max, T increment = 1) :
		Property(name), min(min), max(max), increment(increment)
	{
		value = min;
		oldValue = value;
	}

	std::string getValue() override { return oneDec(value); }
	std::string getRange() override { return oneDec(min) + "-" + oneDec(max); }

	T getRealValue() { return oldValue; }
